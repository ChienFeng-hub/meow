...
class ConditionalDiagGaussian(BaseDistribution):
    ...
    def get_mean_std(self, z, context=None):
        encoder_output = self.context_encoder(context)
        split_ind = encoder_output.shape[-1] // 2
        mean = encoder_output[..., :split_ind]
        log_scale = encoder_output[..., split_ind:]
...
class MyConditionalDiagGaussian(BaseDistribution):
    ...
    context meaning the variable to condition on
    """
    def __init__(self, shape, loc=None, log_scale=None, SIGMA_MIN=-5, SIGMA_MAX=-0.3, prior_transform=False):
        """Constructor
        ...
        self.prior_transform = prior_transform # set True to perform additional tanh scaling for unlearnable prior.
...
class MyGaussianMixture(BaseDistribution):
    ...
    def __init__(
        ...
        if trainable:
            self.loc = nn.Parameter(torch.tensor(1.0 * loc).to(dtype=torch.float32))
            self.log_scale = nn.Parameter(torch.tensor(np.log(1.0 * scale)).to(dtype=torch.float32))
            self.weight_scores = nn.Parameter(torch.tensor(np.log(1.0 * weights)).to(dtype=torch.float32))
        else:
            self.register_buffer("loc", torch.tensor(1.0 * loc).to(dtype=torch.float32))
            self.register_buffer("log_scale", torch.tensor(np.log(1.0 * scale)).to(dtype=torch.float32))
            self.register_buffer("weight_scores", torch.tensor(np.log(1.0 * weights)).to(dtype=torch.float32))
    ...
    def sample(self, num_samples=1, context=None):
        ...
        # Get samples
        eps_ = torch.randn(
            num_samples, self.dim, dtype=context.dtype, device=context.device
        )
        scale_sample = torch.sum(torch.exp(self.log_scale) * mode_1h, 1)
        loc_sample = torch.sum(self.loc * mode_1h, 1)
        z = eps_ * scale_sample + loc_sample
    ...
    @torch.jit.export
    def get_qv(self, z, context=None):
        q = self.log_prob(z, context=context)
        v = torch.zeros(q.shape, device=q.device, dtype=q.dtype)
        return q, v
